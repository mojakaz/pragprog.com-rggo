...
func TestIntegration(t *testing.T) {
	...
	// Scan hosts
	if err := scanAction(&out, tf, nil, "", false, 1*time.Second); err != nil {
		t.Fatalf("Expected no error, got %q\n", err)
	}
...
func TestScanAction(t *testing.T) {
	...
	if err := scanAction(&out, tf, ports, "40-41", false, 1*time.Second); err != nil {
		t.Fatalf("Expected no error, got %q\n", err)
	}
	// Test scan output
	if out.String() != expectedOut {
...
func TestFilterActionOpen(t *testing.T) {
	// Define hosts for scan test
	hosts := []string{
		"localhost",
		"unknownhostoutthere",
	...
	var out strings.Builder
	// Execute scan and capture output
	if err := filterAction(&out, tf, ports, "100-102", false, 1*time.Second, "open"); err != nil {
		t.Fatalf("Expected no error, got %q\n", err)
	}
	// Test scan output
	if out.String() != expectedOut {
...
func TestFilterActionClosed(t *testing.T) {
	// Define hosts for scan test
	hosts := []string{
		"localhost",
		"unknownhostoutthere",
	...
	var out strings.Builder
	// Execute scan and capture output
	if err := filterAction(&out, tf, ports, "100-101", false, 1*time.Second, "closed"); err != nil {
		t.Fatalf("Expected no error, got %q\n", err)
	}
	// Test scan output
	if out.String() != expectedOut {
